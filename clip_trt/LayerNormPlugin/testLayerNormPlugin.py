...
def getLayerNormPlugin(pluginName):
...
def run(soFilePath,nEmbedding,pluginName):
    ...
    profile.set_shape('inputT',[1,4,nEmbedding],[1024,256,nEmbedding],[1024,256,nEmbedding])
    config.add_optimization_profile(profile)

    pluginLayer = network.add_plugin_v2(inputTensorList, getLayerNormPlugin(pluginName))
...
if __name__ == '__main__':
    ...
    visual_soFilePath       = './VisualLayerNorm.so'
    textual_soFilePath      = './TextualLayerNorm.so'
    visualEmbedding      = 768
    textualEmbedding     = 512

    run(visual_soFilePath,visualEmbedding,"VisualLayerNorm")
    run(textual_soFilePath,textualEmbedding,"TextualLayerNorm")
